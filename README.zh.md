...
`car.py` 是一个**完全无图形**的版本 —— 不开窗口、没有仪表、没有混音台、什么都不画
—— 它按**你真车的转速**演奏你选中的发动机，转速来自插在 OBD-II 口上的 ELM327。
在车内放一只有线小音箱，你的通勤车就有了 RS3 直五、或者一台 V12 的声音。
...
| | |
|---|---|
| `py car.py` | WiFi ELM327（`192.168.0.10:35000`），演奏 8Y RS3 的直五 |
...
- **转速映射** —— 你的车 6500 断油，Aventador 到 8500 才停。`--map stretch`（默认）
  ...
  由车型档案（`--car a3`）给出种子，然后**从你的车实际表现中学习**，所以种子给错了
  也会在一次驾驶内自我修正。
- **用油门踏板，不用节气门** —— 优先取踏板位置 PID：涡轮车上踏板比曲轴**领先
  100–300 毫秒**（这段滞后正是涡轮在建立压力），所以你脚一动声音就起来了。
- **输出缓冲** —— 车载模式什么都不画，没有任何东西会卡住 GIL，所以它要的输出缓冲比
  GUI 小得多（实测 25 ms vs 65 ms）。听到爆音就用 `--host-latency` 调大。
