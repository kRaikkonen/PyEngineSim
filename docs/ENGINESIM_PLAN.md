...
## 16. 和 AngeTheGreat 的 C++ 原版比（2026-09-24，Leo）

Leo：“看看 AngeTheGreat 的 C++ 原版和我们现在处理的版本区别。”
对照的是外层仓库里的 `engine-sim/`（ange-yaghi/engine-sim，~15k 行 C++：src + include，外加 `.mr` 脚本）。

### 16.1 一句话
C++ 原版是**实时把整台机器物理地跑出来**（刚体曲轴机构 + 集总气体网络，10 kHz），声音 = 排气流道的压力
→ 卷积一段**录音/合成的脉冲响应（IR）**。我们是**实时轻量机械模型 + 离线烘焙的原版气体物理**，声音是
**每一级都物理化的合成链**（管道驻波、消音器、亥姆霍兹、材质、辐射、听者位置），不用任何录音。

### 16.2 逐项对照

| | C++ 原版 | PyEngineSim |
|---|---|---|
| 语言 / 平台 | C++17 + CMake，Windows（delta-studio / DirectX），脚本语言 Piranha | Python 3 + numpy（scipy 可选，手机上纯 numpy），pygame；Win/Mac/Linux、Android APK、iOS（Swift 移植音频核心）、OBD-II 车载模式、Forza 遥测 |
| 发动机定义 | `.mr` 脚本，25 台，逐件硬件：凸轮型线（谐波凸轮）、气门升程–流量表（CFM @28″H₂O）、稳压腔/流道容积、每缸主管长度与声音衰减、点火曲线、每条排气的 IR | 131 台 Python 预设（dataclass）：几何与规格，其余由物理推导（VE 模型、BMEP、气门流量按几何合成、涡轮自动匹配） |
| 曲轴机构 | 2D 刚体约束求解器（scs）：活塞、连杆、曲轴都是刚体，默认 10 kHz（个别 30–40 kHz） | 单转动惯量 ODE，气体力经精确曲柄–滑块运动学变成力矩；最多 80 个子步 |
| 气体 | 实时集总网络：稳压腔 → 进气流道 → 气缸 → 排气流道+主管 → 集气管 → 大气，每步 8 个流体子步（80 kHz），带体积动量（动压） | 实时：压缩/膨胀多变过程 + Wiebe 燃烧，燃烧倍数按物理 BMEP 目标每帧标定；**原版那套气体网络逐行移植在 `gas_truth.py`**，离线逐工况烘焙成排气脉冲表和进气流量表（外加离线 1-D 特征线法排气 `gas_moc.py`） |
| 燃烧 | 圆柱形火焰前锋：Metghalchi–Keck 层流火焰速度 × 湍流比（平均活塞速度），每循环随机燃烧效率 | 实时 Wiebe（a = 6.9, m = 2），提前角/燃烧持续随转速；`gas_truth` 里就是原版的火焰前锋；逐缸离散与相位漂移另建模 |
| 混合气 | 化油器式：进气里预混固定空燃比 + 怠速回路，没有喷油/ECU | 闭环空燃比（负荷/转速）、减速断油、逐缸软断火限速（brap）、换挡断火 |
| 增压 | **没有** | 白盒涡轮机（压气机特性图、Greitzer 喘振、转子惯量、废气门 PI、泄压阀/喘振声）、罗茨/离心/双增压、混动、电动涡轮、防迟滞 |
| 排气声学 | **没有管道波动**：每缸只有延迟（长度 / 343 m/s）和 1/L² 衰减；“排气管的味道”全靠卷积 IR | 物理波导：热气体声速的流道延迟、集气合流、四分之一/半波长梳状驻波、三元/中段/尾段、亥姆霍兹、管壁材质振铃、尾管辐射（活塞高通 + 指向性） |
| 进入音频的信号 | 排气流道+主管的表压（+0.1 × 动压），以模拟频率输出、线性插值上采样；**先 1.9 kHz 低通** —— 1.9 kHz 以上全靠 IR 和噪声调制 | 音频速率（32–48 kHz）直接合成，高频由物理决定（排气阀开启的陡峭度、管道、辐射） |
| 合成链 | 抖动 → 10 Hz 隔直 → 导数/原信号混合 → “空气噪声”乘性调制 → **IR 卷积** → 抗混叠 → 电平器 | ~25 级（见 golden 的各级：脉冲、流道、集气、驻波、反射、三元、中段、尾段、亥姆霍兹、金属环、辐射、视角、房间……）→ AGC → 限幅 |
| 听者 | 单声道，没有视角 | 追车 / 驾驶舱（质量定律隔板 + 泄漏 + 结构传声）/ 赛道边（多普勒、ISO 9613-1 空气吸收、地面反射、车身遮挡、风噪、观众距离）/ 车载镜头 |
| 传动与车 | 挡位 + 离合约束 + 车重/风阻/滚阻，手动 | 手动/自动/DCT/单离合/液力变矩器、换挡逻辑、补油、轮胎打滑、测功机曲线 |
| 点火 | 提前角曲线；限速器整机断火一段固定时间 | 提前角/燃烧持续随转速；逐缸软断火 |
| 界面 | DirectX 实时几何（真实的活塞/连杆/曲轴）、示波器（缸压、PV 图、点火提前、气门升程、进排气流量、扭矩/功率、波形）、测功面板 | pygame 发动机舱示意（各种布局，涡轮/机械增压/混动部件动画）、仪表、分析示波器、混音台、中英双语、触屏 |

### 16.3 从原版继承的
- 气体物理本身（`gas_truth.py`：GasSystem、等熵孔口流/壅塞、摩尔–能量–动量输运、火焰前锋、Metghalchi–Keck、
  壁面换热 100 W/m²K、节气门 cos 衰减 + 怠速旁通）——逐行对照 C++ 移植，只在离线跑。
- “声音的源头是排气流道的压力”这个思路，以及“每缸按排气长度延迟”。

### 16.4 原版有、我们没有的
- **实时**的刚体曲轴机构：往复惯性力、二阶力、连杆摆动——我们只有单惯量，往复质量不进力矩。
- **实时**的全气体网络：我们实时跑的是按 BMEP 标定的简化缸压，完整气体只在离线烘焙里。
- 逐件的凸轮型线和气门流量表（我们按几何合成），以及让用户自己写发动机的脚本语言。
- IR 卷积：原版可以直接“换一段录音”得到那条排气的质感和空间感；我们只能靠物理把它算出来。

### 16.5 听感上为什么不一样
- 原版 1.9 kHz 以下是物理，以上靠 IR 和噪声；IR 是固定的，不随转速、负荷、温度、几何变化——录音给了它
  “天然的真实感”，但换材质、换管长、开蘑菇头、换视角都不会自己变。
- 我们每一级都是参数化的物理：转速、负荷、排温、管长、材质、增压、听者位置一变，声音跟着变；代价是没有录音
  那份“白给的真实”，每一分质感都要自己推出来（所以才有 §10–15 这些工作）。

## 17. 真车录音对照：2JZ 单涡轮、涡轮音量、振颤（2026-09-25，Leo）

Leo：“涡轮声音太小了，flutter 仍然不对……增加 supra turbo 2jz”，并给了一段真车录音（`2026-09-25 03-55-20.mp3`，
2JZ 大单涡轮带振颤，22.8 s，四次收油）。对照方法：主音按阶次跟踪（点火线 3 阶的谐波和追踪、按曲轴角重采样），
涡轮哨声和点火线在同一帧里比（不受录音自带 AGC 影响），振颤看 2–8 kHz 包络的衰减、节奏、每一下相对间隙的频谱。

### 17.1 录音告诉我们的
- 哨声就是叶片通过频率：满增压 ~8.7 kHz（~6000 rpm），随增压从 ~4 kHz 爬上来；比点火线低 ~22 dB（−14..−28）。
- 振颤：每秒 10–13 下，前 0.1–0.2 s 和收油前的 2–8 kHz 一样响，0.5 s 内掉 7–18 dB；每一下是亮的（6–8 kHz 最高，
  一直到 16 kHz），不是 1–3 kHz 的“chu”。
- 主音：干净的 3/6/9 阶直六；1.5 阶约 −25 dB（我们原来 −4..+12，“一顿一顿”）。

### 17.2 从硬件、从物理改的（不是调数）
- **2JZ 单涡轮（`2jzst`）**：Precision 7675 Gen2（厂家公布 76.7/102.4 mm），新字段 `turbo_d2_mm` —— 点名的
  涡轮直接用它的轮径，热端仍按满增压转速解；满增压 5000 rpm（车主报 0.96 A/R “5k 中后段”，加速中再晚 ~900）。
  **预测**（没有拟合）：哨声 4900 rpm 5.0 kHz / 5360 6.3 / 5880 7.6 / 满增压 8.0 kHz @ 6135，真车 5.2/6.0/7.0/7.7–8.8。
- 管式 6-1 排气歧管 `header_equality=0.85`（自动分类给公路车的是铸造 log：0.43 m 长差，前后缸交替早晚 → 假 1.5 阶）。
- 中冷+管路 `charge_air_l=10`（新字段）：深喘振周期 ∝ 容积 / 压气机流量，从录音的 10–13 Hz 反推，JDM 前置中冷合理。
- **喘振时叶轮的回流搅拌功** `C_RC`：低于喘振线时叶轮在自己内部搅空气，只发热不增压（再循环寄生损失，
  Harley et al.；径向泵关死点耗 40–60 % 最高效率点功率，取下限）。之前 W=0 时只有盘摩擦，涡轮一直转、一直补压、
  振颤停不下来；现在 7675 收油 0.5 s 掉 −7 dB，在真车四次（−7..−18）之内；满增压转速曲线一点没变。
- **管道模态截止**：失速叶片的力叶片间不相关，截止频率 f10 以下只有平面波能出去（1/z 功率，−7.8 dB）；
  **管道环鸣**按 Pai 的原意改回：截止处比宽带高 10 dB 的**峰**，不是宽带能量的 9 倍（之前多了 ~12 dB，就是那个“chu”）。
- **原厂空气盒的高频**：集总亥姆霍兹只在箱子比波长小时成立；第一横向模态（c/2L）以上它是膨胀腔+滤芯，
  压气机噪声只低 ~12 dB。之前 5 kHz 被砍 ~80 dB，所有空气盒振颤车（F40…）只剩低频“咚”。

### 17.3 用录音标定的（一个数一个理由）
- 蘑菇头的叶片音 `_TB_TONE_POD=1.43`：哨声比点火线低 22 dB（追尾视角，经典音色）。
- 原厂空气盒 `_TB_TONE=0.36`：比蘑菇头低 12 dB —— 文献里 OEM 进气对压气机高频降 10–26 dB 的下限
  （OSU：专门的消音器 8.6 kHz 降 26 dB）；原来 0.160 是凭耳朵（Leo：“其他涡轮车还是有点小”）。
  1×/2× 轴频不跟着蘑菇头放大（真 7675 录音里看不到 1× 线）。
- 喘振总量 `_TB_SURGE` 10 → 32：定义不变（S15 在 0.93 bar/5800 rpm 收油，前 0.5 s 比全油门低 −11 dB(A)），
  是因为 17.2 把环鸣和截止以下的失速拿掉了才重解；同一个数让 2JZ 的头几下正好落在真车位置（≈ 收油前 2–8 kHz）。

### 17.4 听感修正（Leo 的耳朵）
- **“像个铰接吸尘器”**：一度把 2JZ 单涡轮换成物理音色、再给整层涡轮 +20 dB 平衡（物理音色的 makeup 在 F2004 上定，
  对涡轮层差 ~20 dB）——吸尘器就是这层（哨声+whoosh+叶尖噪声一起 +20 dB）。已撤回：车回经典音色，平衡常数删掉。
  物理音色的 1.5 阶对得上真车，但太干净；经典音色的 1.5 阶还偏高（来自每缸“个性” ±27 % 和端口混响的梳状），
  这两个是全车队的耳调元素，没动，留给 Leo 决定。
- **“振颤在压力释放后不停”**：经典 AGC 的收油上限是 `max(2.2, 点火时增益)`，大声的车追尾时点火增益远低于 2.2，
  收油时增益最多涨 17 dB，把衰减中的振颤又抬回满音量。现在喘振/泄压进行中 AGC 不许升增益（物理音色本来就这样）。
  输出端 2JZ 收油后 2–8 kHz：0 s −1.4、0.2 s −7、0.4 s −11、0.6 s −13.5、1.0 s −17 dB（真车 +3..−0.4、−10..+2、
  −15..−3、−17..−6、−18..−8）。

### 17.5 第二轮（Leo：“flutter 为什么有低音打鼓；节奏对了但不是 whistle 感的尖声；其他涡轮车没变化”）
- 拆开测（2JZ 收油后 0.6 s，追尾，相对同频段的发动机）：65 % 的能量在 200–500 Hz —— 气流单极子（每次倒流
  一团空气进出滤芯口，精确的物理）敲响蘑菇头短管的四分之一波（25 cm → ~300 Hz，每下响 ~14 ms）= 打鼓；
  另 11 % 是 0.7 f_s 旋转失速的“咚”（~900 Hz）。真车录音在这两段每下的对比度只有 ~0–3 dB。
- **旋转失速只在正向失速里**（0 < φ < φ_z）：深喘振是整圈一起倒流，没有跟着叶轮转的失速团（原来跟着每次翻转响）。
- **失速叶片的脉动力系数取合理范围上限 0.3**：真车每一下全是高频；单极子是精确的，不动它。
- **叶尖间隙噪声（TCN）**：亚声速时它是离心压气机的**主导**噪声（Raitor & Neise 2008），我们原来比叶片线低
  30–42 dB → 改为与（实测标定的）叶片线**等功率**（“主导”最保守的读法）、窄带（Q 20：真车那团 ~300 Hz 宽）、
  倒流时关——每个“tu”里带一声哨。小涡轮的叶片频率 13–20 kHz 听不到，你听到的哨就是它（半个叶片频率，7–9 kHz）。
- **2JZ 单涡轮的蘑菇头装在压气机盖上 10 cm 短接头**（新字段 `pod_pipe_m`，大单涡轮的典型装法；录音里没有鼓）：
  ~300 Hz 的“咚”变成 ~620 Hz 的短“哒”，200–500 Hz 占比 50 % → 10 %。其他蘑菇头车仍按 25 cm。
- `_TB_SURGE` 按定义再解 32 → 25（S15 前 0.5 s 仍是 −11 dB(A)）。
- 结果：涡轮层（A 计权，相对发动机）A3 −26 → −16，FK8 −40..−27 → −28..−20，S15 −41..−28 → −18..−12；
  2JZ 每下 4–11 kHz 对比度 +11..+14（真车 6–8 kHz +10..+15）。
- 还差：2JZ 每一下的峰在 2–3 kHz（管道环鸣 2.1 kHz + 失速带），真车在 6–8 kHz（那团 ~0.83×叶片频率的音）。

### 17.6 第三轮（Leo：“节奏完全正确，但为什么不是高 pitch 口哨声越来越弱；松油门又比踩油门小了”）
- **松油门变小**：上一轮“喘振时 AGC 不许升增益”把整车的收油都压住了（Leo 之前明确说过“阀门关闭之后声音
  变小，不要这样”）。改为：AGC 照旧把**发动机**拉回来；**涡轮层**（哨声、振颤、泄压）保持点火时的增益、
  3 s 慢慢放开——它们自己衰减，不被抬回满音量。输出：收油后整体 −6/−9/−6/−4 dB（0/0.2/0.5/1.0 s），原来 −15..−19。
- **为什么不是高音哨**：拆开看，收油时（叶尖变亚声速）半个叶片频率的叶尖间隙带自动打开，比真正的哨声（叶片
  频率，随轴速下降）高 5–10 dB → 听成 3–4 kHz 的中音。叶尖间隙噪声是**稳定近失速正向流**的旋转不稳定，
  深喘振里（整圈倒流 / 远在喘振线右侧回灌）形成不了——同旋转失速团一样关掉。真车录音振颤里也没有半频带。
- 上一轮把失速叶片力系数提到 0.3 是看错了证据：真车亮的是 6–8 kHz 的**哨声带**，失速噪声物理上落在 2–4 kHz
  → 退回 0.1。
- **管道环鸣**要有管道才会响（Pai 是长直管）：蘑菇头直接装在压气机盖上（管长 ≈ 直径）就没有——2JZ 单涡轮
  真车没有 2.1 kHz 的线。按管长 / 直径从 0 过渡到 1（两个直径以上全量；默认 25 cm 管和原厂风道不变）。
- `_TB_SURGE` 改为**用真车录音定**：2JZ 单涡轮收油头 0.1 s 的 2–8 kHz 比收油前高 +1.3 dB（四次收油平均）→ 60。
- 试过“间隙噪声在全叶片数的一半（= 主叶片频率）”这个读法（Raitor & Neise 的叶轮是 13+13）：振颤几乎不变、
  FK8 的哨声 −4 dB，没采用。
- 还差：我们每个“tu”开头是一个宽带“咔”（集总模型里气流 ~1 ms 翻转），真车是软的、亮的；翻转快慢由 Greitzer
  的等效管长决定（估计值），它同时决定振颤节奏（现在是对的）——下一步从几何推它，不凑。
  （golden 里 rs3 的排气级变 0.9 dB 不是 bug：valve bypass 的“blow-out”读上一块的输出电平。）

### 17.7 第四轮：泄压就是哨声（Leo：“直接把已有的 turbo whistle 当作泄压的声音”；“叶尖声音降低”；“叶片音量滑槽”；
“松油门那一瞬间泄压应该比后续大，有放屁声”；“松油门引擎声还是变小”；“涡轮和 cabin、flyby 整合不好”）
- **振颤 = 现有的哨声被每次倒流切断**：真车录音里收油时哨声带比全油门时的哨声**高** +2.7/+7.8/+7.9 dB（每次
  叶轮重新抓住气流被点亮一下）。我们的哨声门控（只在正向流量达到喘振线才全开）让它在振颤里几乎一直关着 → 改为
  “只有倒流时才关”（零流量时叶片在搅空气，仍然受载），喘振期间点亮 `_TB_FLUTTER=5.5`（测得：2JZ 收油哨声带
  比全油门哨声 +4.7 dB，真车中值 +5），点亮强度 ∝ 压力摆幅 ~U2²（随轴速衰减更快）。“喘振中”用**倒流**判定
  （孪生模型的 busy 在大涡轮全油门时就一直亮，会把全油门哨声也放大）。振颤期间哨声保持收油那一刻的音色。
- 叶尖间隙带 −10 dB（Leo 的耳朵，`_TB_TCN=0.316`）；新混音滑槽“涡轮叶片哨声” `blade_vol`（叶片线 + 叶尖带 +
  收油时点亮的哨声，默认 1 = 实测/物理电平）。
- **第一下“噗”**：满增压第一次倒灌，喷流 ~U³ 比后面大；低频气团“咚”每下都差不多（线性泄压的来源）→ 喷流
  取范围上限 0.3，单极子 ×0.25（`_TB_MONO`：集总模型 ~1 ms 翻转，真车每下 1 kHz 以下 ≤3 dB），`_TB_SURGE=60`：
  第一下 2–8 kHz 比全油门 +9 dB，之后 +4/+1/−2.5/−4…（真车 +3..+10 然后衰减）。
- **松油门不许变小**：去掉泄压时对引擎的压低（它压了整整一个振颤）；AGC 只在**油门关闭时**：电平估计快速下降、
  增益立即跟上、上限放到点火增益的 10 倍（封顶 12）——驾驶舱点火时 ~4.5 倍，原来收油无处可升。涡轮层仍按点火
  增益（喘振期间不放开），振颤和泄压不会被抬回来。踩油门时一切照旧（golden 里自然吸气车的加速逐位相同）。
  结果：收油后整体 追尾 −3/−0.4/+0.8/−0.2 dB，驾驶舱 −1.3/−2/+0.6/+1.2 dB（0/0.2/0.4/1.0 s）。
- **涡轮的进气噪声从进气口辐射**（场边：进气口的位置和朝前指向性；敞篷车载：airbox 路径），原来被当成发动机
  壳体。追尾、封闭驾驶舱不变。
- 场边回声（Leo 问“越远回声延迟越长？”）：回声延迟由**墙**的位置决定——对面护墙（线外 15 m）~87 ms、身后建筑
  （5 m）~29 ms，与你站多远无关；地面反射离得越远延迟越**短**；只有身后墙和对面护墙之间来回的那条随距离变长
  （4 m 140 ms → 40 m 350 ms）。随距离变的是回声的**相对音量**（近处直达声高 ~20 dB，远处相近，混响场 RT60
  1.5 s 越来越明显）——模型就是这么算的。
- 发现：场边“换观众”那一刻（车在两个观众中点）延迟连续但多普勒从远去 ×0.9 跳到驶来 ×1.13，整车音调跳一下，
  哨声最明显——下一步在切换点对两个观众做交叉淡化。

- **场边回声更明显**（Leo：“远了要有长回声，近了短回声，回声要是发出那一瞬间的声音、带那一刻的高低频衰减”）：
  每条回声本来就是按它自己那条路径的**推迟时间**读同一个发射声（自己的多普勒），并按路径长度走 ISO 9613-1
  的空气吸收。缺的是一个大反射面 → 场地加一座**赛道对面的看台/维修区建筑**（线外 60 m、12 m 高、混凝土 R 0.8）：
  车经过时回声 350 ms，站 4 m 比直达声低 32 dB（被盖住）、12 m −23、40 m −14 dB（清楚可闻）；车在 150 m 外时
  130–190 ms、只低 4–5 dB；多走 240 m 空气，3.3–3.9 kHz 就 −3 dB（直达声 7–24 kHz）——远处的长回声是暗的。
  身后的墙改成实心混凝土（R 0.6 → 0.85）：近处 29 ms 的短回声 +3 dB。

### 17.8 没解决 / 下一步
- 真车收油后有一团 ~0.83×叶片频率的窄带音（0.1–0.4 s），文献的叶尖间隙噪声在 ~0.5×，没加（不硬凑）。
- 物理音色给涡轮车用：发动机绝对电平要从求解器的绝对流量来（旧的开放项），才能和经典层级自然平衡。
- 原装“9”（2JZ-GTE 顺序双涡轮）的歧管同样被分成铸造 log；真车是两个 3-1 铸造歧管各接一个涡轮——待改。
- 回流力矩（Fink 1992 自由转子喘振模型）没查到原文，现在的 0.5σ 保留。

## 18. 高转 F1 为什么没劲（2026-09-25，Leo：“其他车很好了，但为什么高转的 F1 引擎表现如此之差；SF25 标定 15000，比赛只用到不到 13000”）

- **摩擦**：燃烧扭矩没问题（F2007 18500 rpm 燃烧 766 hp，SF25 15000 rpm 807 hp），是摩擦吃掉了一半以上——
  F2007 的二次系数是 F2004 的 6 倍（18500 rpm 179 N·m，FMEP 9.4 bar），SF25、MP4/4 用的是公路车默认值
  （FMEP 19 / 15 bar）。摩擦由**活塞平均速度**决定（FMEP ~ c_m、c_m²），短冲程的 F1 在 19000 rpm 的活塞速度
  和公路车红线差不多：以台架已对上真车的 F2004 为准，按排量和冲程换算（s' = s·V'/V，l' = l·(V'/V)(S'/S)，
  q' = q·(V'/V)(S'/S)²）→ FMEP 2.2–2.4 bar。F2007 19000 rpm 273 → 661 hp；MP4/4 659 hp（真车比赛 ~650）。
- **SF25：燃油流量上限**（FIA：10500 rpm 以上 100 kg/h，以下 0.009 N + 5.5 kg/h）才是整台动力单元的约束：
  100 kg/h × 43 MJ/kg × ~50 % 制动效率 = 597 kW（~800 hp），10500 以下 ~543 N·m（`power_limit_kw`/
  `torque_limit_nm`）。增压 1.6 → 2.8 bar 表压（真车 3.5–4 bar 绝对）。新属性 `prechamber_ignition`：射流点火
  让末端气体来不及自燃，高压缩比高增压不爆震（`bmep_model`：无爆震推迟）。压缩比仍写 13：真车 ~18，但模型把
  ≥ 14.5 当柴油机（空燃比 25）。结果：8000 610、10000 763、11000–15000 平 801 hp——再往上转不加力，自动挡
  12000 换挡（真车比赛在 13000 以下）。
- 还差：低挡升挡时转速冲到 ~13300（换挡过程中转速继续上升）；F2007 661 hp 比真车 ~750 低（燃烧侧 ~10 %）；
  公路车默认摩擦也是按转速平方而不是活塞速度——短冲程高转的公路车（S2000、摩托）同样偏大，未动。

### 18.1 第二轮（Leo：“都给我解决了 f2004 f2007 还有 sf25”）

- **进气道**：`map_model` 的全开节气门损失是全车队共用的一个“公路进气道”（滤芯 + 进气管 + 一片节气门，
  K_BALANCE），红线处总是 ~0.90 atm——F2004/F2007 的燃烧侧少的 10 % 就在这里（VE 平的，1.11–1.15）。
  K_BALANCE 换成米制：a = 1 就是 Cd·A_nom = Vd·redline / (120·√(R·T0)·K)（2.0 L、7000 rpm 的车 = 30 mm 孔）。
  F1 是每缸一个节气门/喇叭口直通冲压进气箱：新属性 `throttle_bore_mm`，开口 n·π/4·d²·Cd（喇叭口 Cd 0.97）。
  口径由气门推：法拉利 tipo 049 进气门 40.4 mm ×2，喉口 0.87×，合成一个口 √2×0.87×40.4 = 50 mm（F2007 按缸径
  98/96 → 51 mm）→ 6.6 倍公路进气道，红线 MAP 0.90 → ~1.0。踏板是**扭矩需求**（F1 线控）：ECU 开喇叭口使踏板
  拿到和公路车一样比例的全开进气，所以怠速锚点、半油门、补油都不变，只有顶端抬起（`Simulator._itb_wot_gain`）。
  结果：F2004 18000 rpm 903 hp（真车 ~865–900），F2007 19000 rpm 754 hp（真车 ~750）。
- **升挡冲转**：换挡后的 0.18 s 防抖锁是防“升-降-升”的；按车速在当前挡已到换挡点的升挡不可能是抖动，允许在锁内
  发生（降挡仍受锁）。SF25 2 挡 ~3 g、~10000 rpm/s，原来冲到 13375 → 现在每个挡 12000–12033。
- **风阻和齿比**：全车队共用公路车的 CdA 0.31×2.2 = 0.68 m²，F1 在顶挡一直顶到限转。新属性 `drag_cda`，由真车
  最高速和功率反推（胎面功率扣滚阻 = ½ρ·CdA·v³）：F2004 蒙扎 ~365 km/h / ~900 hp → 1.06 m²，终传 5.35 → 4.95 让
  7 挡在峰值功率（18000）碰到最高速；F2007 ~352 km/h / ~750 hp → 0.99 m²（7 挡本来就在 ~19000）；SF25 ~345
  km/h、597 kW + 120 kW MGU-K → 1.34 m²，燃油流量上限让 11000 以上功率平，所以变速箱按**换挡点**定：8 挡在
  12000 碰到最高速（4.33 总传动比 → 0.866），各挡从 1 挡几何分布。全油门到极速：F2004 365 km/h @ 18041，
  F2007 348 km/h @ ~18900，SF25 346 km/h @ 12061（全程最高 12159，<13000）。
- **SF25 压缩比 18**（FIA 2014–25 上限）：新属性 `diesel`（None = 旧规则 CR ≥ 14.5），`Engine.is_diesel()`
  取代了 6 处 CR 判断；SF25 写 `diesel=False`。
- 不变：公路车 golden 对 36b876b 全部 25 级 +0.00 dB；test_obd、test_headless 通过。
- 没动：MP4/4 是手动箱，自动模式升挡时离合分离期间发动机空冲 ~850 rpm（手动/单离合设计上的“空冲 + 踢”）；
  默认不开抓地力模型（Slip 开关），所以 F1 0–100 只要 ~1 s（真车 ~2.5 s，受抓地力限制）。


## 19. 场边飞驰不够“粗犷狂野”、像电动车（2026-09-25，Leo）

- 对照 Leo 的真 Aventador 飞驰录音（300 Hz 以上，手机）：真车**驶来**时 1–2.5 kHz 只比 300–1k 低 3.0 dB、谐波之间是
  密的噪声底（谐噪比 −8.8 dB）；我们驶来时暗 5 dB、干净的谱线、谐噪比 +8.7 dB——就是“电动车的哨声”。驶离时反过来：
  真车更暗（−4.8/−12.6），我们更亮（−1.1/−5.1）。
- 逐项关掉（车身遮挡 / 尾管指向 / 射流静区 / 墙 / 场 / 距离 4–12 m）：没有一项能让驶来的谐噪比变负。真车驶离
  （尾管对着你）本身是偏音调的（+6.4），所以驶来时的粗糙声**不是从尾管口出来的**，是朝侧前方的源：进气、发动机舱、
  排气系统壳体、轮胎。
- 修了三处场边视角的一致性（7 月 10 日场边视角是从当时的追车参数复制的，之后追车的两处修正没带过去）：`g_bay`
  0.45/0.7 → 1（追车的 g_bay 只是它的距离比，场边每个源已经各自按距离传播）；路车发动机舱 `bay_alpha` 0.08 →
  0.22（开底的舱，e24616f）；车身壳体的次低频胸口感 `chassis` 0.30 @ 70 Hz 加进车身源（7336bf9，全向）。
  另外进气箱在第一横向模态以上是扩张腔 + 滤芯（−12 dB），不再按集总亥姆霍兹 −40 dB/十倍频往下掉——喘振那条路
  已经是这样，发动机进气现在一致。
- 效果不大：驶来中频 +1～+4 dB（Aventador、C7、2JZ）；golden 追车/座舱 ≤ 0.02 dB，只有 aven 场边变了。
- 大头还在：进气源本身比排气脉冲低 21–31 dB（重叠的吸气鼓包、没有进气门关闭的压力波），再过气箱和进气口又低
  18–27 dB，辐射出来比排气低 ~60 dB——真车进气一般只低 10–25 dB。2JZ 驶来时主要是涡轮哨声（纯音），也是
  电动车感的来源。下一步要 Leo 的耳朵定：进气声的物理源（进气门关闭的水锤波）会改所有车所有视角。
...
- §17：Precision 7675 Gen2（76.71/102.36 mm）<https://www.full-race.com/store/precision-turbo/precision-billet-cea/precision-pte-gen2-pt7675-cea-turbo>；7675 + 0.96 A/R 满增压“5k 中后段”<https://www.supraforums.com/threads/details-on-gen2-7675-and-7685.974121/>；
  叶尖间隙噪声在 ~0.5 BPF：Raitor & Neise 2008（见上）；汽油涡轮 whoosh 4–13 kHz、下限是第一个高阶模态的截止：
  <https://asmedigitalcollection.asme.org/gasturbinespower/article/145/11/111003/1166560/Physical-Insight-Into-Whoosh-Noise-in-Turbocharger>；
  再循环是只加温不增压的寄生功：<https://www.researchgate.net/publication/320251992_Characterising_the_Influence_of_Impeller_Exit_Recirculation_on_Centrifugal_Compressor_Work_Input>；
  OEM 压气机高频消音 8.6 kHz 降 26 dB：<https://mae.osu.edu/sites/default/files/2022-05/J113.pdf>；Garrett GTX Gen II 是 10 叶轮 <https://www.garrettmotion.com/racing-and-performance/performance-catalog/turbo/gtx3582r-gen-ii/>
